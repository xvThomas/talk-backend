...
func routeReq(t *testing.T, router mcplegacy.McpRpcRouter, id int, method string, params any) mcplegacy.JSONRPCResponse {
	t.Helper()
...
func TestRouter_Initialize_CapabilitiesPrompts(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildToolsWithPrompts())
...
func TestRouter_PromptsList_Empty(t *testing.T) {
	...
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildToolsWithSchemaError())
...
func TestRouter_PromptsGet_NilParams(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildToolsWithPrompts())
	req := mcplegacy.JSONRPCRequest{JSONRPC: "2.0", ID: json.RawMessage("1"), Method: "prompts/get"}
...
func TestRouter_ToolsCall_UnknownTool(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildTools())
...
func TestRouter_NotificationCancelled_NoError(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildTools())
	req := mcplegacy.JSONRPCRequest{JSONRPC: "2.0", Method: "notifications/cancelled"}
	resp := router.RouteRequest(context.Background(), req)
	if resp.Error != nil {
...
func TestRouter_ToolsList_OutputSchemaError(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildToolsWithOutputSchemaError())
	resp := routeReq(t, router, 1, "tools/list", nil)
	if resp.Error == nil {
...
func TestRouter_ToolsCall_ExecuteError(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildToolsWithExecuteError())
	resp := routeReq(t, router, 1, "tools/call", callRequest{
		Name:      "sum",
...
func TestRouter_ToolsCall_MarshalError(t *testing.T) {
	router := mcplegacy.NewMcpRpcRouter("s", "1", buildToolsWithMarshalError())
	resp := routeReq(t, router, 1, "tools/call", callRequest{
		Name:      "sum",
		Arguments: map[string]any{"a": 1, "b": 2},
	})
