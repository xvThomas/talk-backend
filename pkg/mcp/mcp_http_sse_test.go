...
func TestHTTPSSE_SumTool(t *testing.T) {
	...
	post := func(t *testing.T, id int, method string, params any) mcp.JSONRPCResponse {
		t.Helper()
	...
	if callResult.Content["sum"] != 11 {
		t.Errorf("expected sum=11, got %v", callResult.Content["sum"])
	}
}
